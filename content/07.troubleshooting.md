...
Software errors can be the result of syntax errors, dependendy issues, operating system conflicts, bugs in the software, problems with the input data, and many other issues.
Narrowing down the source of the error is helpful when trying to resolve it.
Running the software on the test data that comes with the software helps narrow the possibility of error sources -- if the test data successfully runs, the command is likely free of syntax errors, the source code is not completely broken, and the tool is likely interacting appropriately with dependencies and the operating system. 
...
If there is a GitHub/GitLab/Bitbucket issue tracker for the software, or a Gitter, slack, or google groups page, performing a targeted search with the error message may provide additional context or a solution for the error.
...
When searching for the error message and reading the documentation do not resolve an error, it is usually appropriate to for help either from the software developers or from a bioinformatics community.
...
At minimum, include the name and version of the program, the method used to install it, whether or not the test data ran, the exact code that produced the error, the error message, and the full output text from the run (if any is produced additional to the error message).
