...
Workflow systems both simplify and improve computational project management, but even workflows that are fully specified in workflow systems require additional investment to stay organized and documented. (CTB note: maybe refer to version control here, and say that it's discussed below?)
...
The DAG shown in this figure illustrates a real bioinformatics workflow from (CTB: SGC) and was generated by modifying the default Snakemake workflow DAG [@doi:10.1101/462788].
...
Be respectful of people's time and energy: post in the relevant workflow forums only when you have hit a stopping point you are unable to work through. (CTB: suggest differentiating between local and global here. Maybe don't discourage people from posting in local?)
