...
Fortunately, with a little direction/tending/care, workflow systems both simplify and improve computational project management.

### Documenting workflows for yourself and others

The coded portions of workflows are self-documenting, with each analysis step (and parameters) recorded within the workflow-specific syntax.
...
In some cases, there may be portions of the workflow that you have not been able to automate, such as downloads of data from protected servers (e.g. containing protected identity information), or manual data and metadata cleaning steps.
In these cases, it is easy to lose track of which parameter combinations were used to generate a particular set of output files.
For these steps, it is essential to keep rigorous documentation alongside your workflow that would enable you to entirely reproduce the manual steps.

Below, we present some tools and strategies for improving workflow documentation.
While some of tools discussed below will inevitably change over time, these principles apply broadly and will help you design clear, well-documented, and reproducible analyses. What is most important is to develop a clear set of strategies and implement with them tenaciously.
...
There are a myriad of ways to document your computational work, and you'll need to experiment with the ways that work for you.
...
### Computational Notebooks (a subset of viz? or of documentation?)
...
#### Develop your workflow using subsampled data
